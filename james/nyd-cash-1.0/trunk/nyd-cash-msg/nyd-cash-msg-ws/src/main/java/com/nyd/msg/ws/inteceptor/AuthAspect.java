package com.nyd.msg.ws.inteceptor;
...
import com.nyd.msg.model.LoginInfo;
import com.nyd.msg.service.AuthService;
...
public class AuthAspect {
     ...
     * 定义校验切入点
     */
    @Pointcut("execution(* com.nyd.msg.ws.controller..*(..))")
    public void verify() {
    }
    ...
    private boolean doVerify(Map<String,String> params){
        ...
        if(token != null && token != "" && accountNumber != null) {
        	...
        	if(deviceId != null && accountNumber != null){
        		return authService.judgeTimeout(accountNumber,deviceId);
        	} else {
        		return false;
        	}
        }
    ...
    private boolean judgeTimeoutByMd5(LoginInfo check) {
		if (StringUtils.isBlank(check.getUserToken())) {
			...
			try {
				LoginInfo source = (LoginInfo)JSON.parseObject(redisService.getString(LOGIN_TOKEN_PREFIX + check.getAccountNumber() + ":" + check.getUserToken()), LoginInfo.class);
    ...
    private boolean checkToken(LoginInfo checkToken, LoginInfo source) {
		...
		if(!StringUtils.isEmpty(checkToken.getUserId()) && !checkToken.getUserId().equals(source.getUserId())) {
			return false;
		}
		return true;
	}
