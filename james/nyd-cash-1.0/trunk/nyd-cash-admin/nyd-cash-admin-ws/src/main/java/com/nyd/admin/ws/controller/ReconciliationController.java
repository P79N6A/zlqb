...
import com.nyd.admin.service.excel.hanlder.ReadHandler;
import com.nyd.admin.ws.utils.Constants;
...
public class ReconciliationController {
    ...
    @Autowired
    private ReconResultService reconResultService;
    ...
    @PostMapping("/queryReconResult")
    public ResponseData queryResult(@RequestBody ReconQueryVo vo){

        try {
            return ResponseData.success(reconResultService.findPage(vo));
    ...
    @PostMapping("/queryBillWsm")
    public ResponseData queryBillWsm(@RequestBody BillWsmVo vo){
        String tableName = Constants.BILL_WSM_PREFIX+vo.getReconciliationDay().replace("-","");
        try {
            return ResponseData.success(billWsmService.findPage(vo,tableName));
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return ResponseData.error();
    ...
    @PostMapping("/queryChargebackWsm")
    public ResponseData queryChargeWsm(@RequestBody ChargebackWsmVo vo){
        String tableName = Constants.CHARGE_BACK_WSM_PREFIX+vo.getReconciliationDay().replace("-","");
        try {
            return ResponseData.success(chargebackWsmService.findPage(vo,tableName));
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return ResponseData.error();
    ...
    public ResponseData importExcel(@RequestParam("file") CommonsMultipartFile file,@RequestParam("fundCode") String fundCode,@RequestParam("reconDate") String reconDate) throws Exception {
...
//            System.out.println(JSON.toJSONString(chargebackWsmVos));
    ...
    @RequestMapping(value = "/exportRemitReport",method = RequestMethod.GET)
    public void exportRemitReportExcel(String startDate,String endDate,HttpServletResponse response) throws Exception {
        List<RemitReportVo> reportVos = financeService.queryRemitReportVo(startDate,endDate);
        ExcelKit.$Export(RemitReportVo.class, response)
                .toExcel(reportVos, "放款对账单");
    }

    @RequestMapping(value = "/exportRepayReport",method = RequestMethod.GET)
    public void exportRepayReportExcel(String startDate,String endDate,HttpServletResponse response) throws Exception {
        List<RepayReportVo>  reportVos = financeService.queryRepayReportVo(startDate,endDate);
        ExcelKit.$Export(RepayReportVo.class, response)
                .toExcel(reportVos, "还款对账单");
    }
