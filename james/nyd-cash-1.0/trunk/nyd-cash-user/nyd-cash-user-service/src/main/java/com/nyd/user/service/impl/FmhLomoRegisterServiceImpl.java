...
@Service(value ="fmhLomoRegisterService")
public class FmhLomoRegisterServiceImpl implements FmhLomoRegisterService{
	
	private static Logger logger = LoggerFactory.getLogger(FmhLomoRegisterServiceImpl.class);
		
	public static String APPNAME = "fmh";
	
	public static String CHANNELID = "fmh_lomo";
	
	public static String NYD_CHANNELID = "nyd_lomo";
	
	public static String WWj_CHANNELID = "wwj_lomo";
	...
	@Override
	public UniteRegisterResponseData fmhLomoRegisterReq(UniteRegisterDto dto) {
		UniteRegisterResponseData  responseData  = UniteRegisterResponseData.success();
		logger.info("fmh lomo register param"+JSON.toJSONString(dto));
		...
		try {
			...
			logger.info("select accout error",e);
		...
		if (null != accountList && accountList.size() > 0) {
            ...
            if(StringUtils.isNotBlank(account.getSource()) && (account.getSource().contains(NYD_CHANNELID) 
            	 || account.getSource().contains(WWj_CHANNELID)	)) {
		...
		try {		
			if(!this.registerUser(dto.getMobile(), dto.getChannelId(),dto.getAppName())) {
				logger.error("fmh 用户 注册 error mobile:" + dto.getMobile());
				responseData = UniteRegisterResponseData.error(UniteRegisterRespCode.SERVER_ERROR);
				return responseData;
			}			
	   }catch(Exception e) {
