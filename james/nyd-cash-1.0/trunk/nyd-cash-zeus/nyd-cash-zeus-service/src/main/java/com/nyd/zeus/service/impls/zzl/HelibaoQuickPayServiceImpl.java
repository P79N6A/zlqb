...
import com.nyd.zeus.model.helibao.util.MessageHandle;
...
public class HelibaoQuickPayServiceImpl implements HelibaoQuickPayService{
	...
	public QuickPayCreateOrderResponseVo quickPayFirstPayPreOrder(QuickPayFirstPayPreOrderVo requestVo) {
		...
		HelibaoFilesConfigVO config = queryHelibaoFilesConfigInfo();
  ...
  /**
   ...
   //----------------异步通知接口
		...
		try {
			...
			if (!MessageHandle.checkSign(requestVo,config.getCertPath())) {
				...
				if("SUCCESS".equals(resultStatus) || "FAILED".equals(resultStatus)){
			        ...
			        bankSerialService.updateOrderInfo(requestVo.getRt5_orderId(), status);
			         //查询处理中展期数据，如果成功，则调用展期成功接口
			        ExtensionRecordVO extensionRecordVo =  extensionRecordService.selectByOutTradeNo(requestVo.getRt5_orderId());
					if (!ChkUtil.isEmpty(extensionRecordVo)) {
							extensionRecordVo.setExtensionStatus(status);
							extensionRecordService.generateExtensionDatas(extensionRecordVo);
						}
				}
			}
		} catch (Exception e) {
	...
	private HelibaoFilesConfigVO queryHelibaoFilesConfigInfo(){
		HelibaoFilesConfigVO helibaoFilesConfig = new HelibaoFilesConfigVO();
		//String pubPath = getClass().getClassLoader().getResource("").getPath();
		/* URL fileURL=this.getClass().getResource("/helibaoFiles/sit/");   
	    String pubPath = fileURL.getFile();  */
	   // log.info("配置文件路径:"+pubPath);
		helibaoFilesConfig.setCertPath("");
		helibaoFilesConfig.setCustomerNumber("C1801121010");
		helibaoFilesConfig.setPfxPath("");
		helibaoFilesConfig.setPfxPwd(helibaoProperties.getPfxPwd());
		helibaoFilesConfig.setPayIp(helibaoProperties.getPayIp());
		helibaoFilesConfig.setPayUrl(helibaoProperties.getPayUrl());
		helibaoFilesConfig.setPayCallbackUrl(helibaoProperties.getPayCallbackUrl());;
		helibaoFilesConfig.setEntrustedCallbackUrl(helibaoProperties.getEntrustedLoanUrl());
		helibaoFilesConfig.setEntrustedLoanUrl(helibaoProperties.getEntrustedUploanUrl());
		helibaoFilesConfig.setEntrustedUploanUrl(helibaoProperties.getEntrustedCallbackUrl());
		helibaoFilesConfig.setSignkey(helibaoProperties.getSignkey());
		helibaoFilesConfig.setSignkeyPrivate(helibaoProperties.getSignkeyPrivate());
		helibaoFilesConfig.setDeskeyKey(helibaoProperties.getDeskeyKey());
		helibaoFilesConfig.setGoodsName(helibaoProperties.getGoodsName());
